...
class PDFEditor:
	...
	def _build_ui(self):
		...
		# add text toggle
		self.add_mode = False
		self.add_btn = ttk.Button(toolbar, text="🖊 Aggiungi testo", command=self.toggle_add_mode)
	...
	def _on_canvas_click(self, event):
		...
		# verifica se click è dentro un Text widget -> ignora drag casella
		widget = event.widget
		if isinstance(widget, tk.Text):
			return  # lascio al Text gestire input

		# altrimenti check handle o casella
	...
	def _on_canvas_drag(self, event):
		if self.selected_box is None or self.dragging is False and self.resizing is False:
			return
